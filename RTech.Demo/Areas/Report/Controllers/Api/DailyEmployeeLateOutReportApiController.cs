...
{
    public class DailyEmployeeLateOutReportApiController : ApiController
    {
        [HttpPost]
                ...
                switch (vm.Filter.Filters[0].Operator.ToLower())
                {
                    case "startswith":
                        reportData = (from c in result
                                      ...
                                      select c).Where(x => x.LateOut != "00:00" && x.LateOut != "").OrderBy(x => x.EmployeeCode).Skip(vm.Skip).Take(vm.Take).ToList();
                        break;
                    case "eq":
                        reportData = (from c in result
            ...
            if (vm.Sort.Count() > 0)
            {
                switch (vm.Sort[0].Field.ToLower())
                {
                    case "employeename":
                        ...
                        {
                            reportData = reportData.Where(x => x.LateOut != "00:00" && x.LateOut != "").OrderBy(x => x.EmployeeName).Skip(vm.Skip).Take(vm.Take).ToList();
                        }
                        else
                        {
                            reportData = reportData.Where(x => x.LateOut != "00:00" && x.LateOut != "").OrderByDescending(x => x.EmployeeName).Skip(vm.Skip).Take(vm.Take).ToList();
                        }
                        break;
                    case "employeecode":
                        if (vm.Sort[0].Dir.ToLower() == "asc")
                        {
                            reportData = reportData.Where(x => x.LateOut != "00:00" && x.LateOut != "").OrderBy(x => x.EmployeeCode).Skip(vm.Skip).Take(vm.Take).ToList();
                        }
                        else
                        {
                            reportData = reportData.Where(x => x.LateOut != "00:00" && x.LateOut != "").OrderByDescending(x => x.EmployeeCode).Skip(vm.Skip).Take(vm.Take).ToList();
                        }
                        break;
                    default:
